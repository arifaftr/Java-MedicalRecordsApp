...
public class FormPemeriksaan extends JPanel {
    private JComboBox<String> cbPasien, cbDokter, cbObat;
    ...
    private Map<String, Integer> obatMap = new HashMap<>();
    ...
    public FormPemeriksaan() {
        ...
        panelInput.add(new JLabel("Obat:")); panelInput.add(cbObat);
        ...
        model = new DefaultTableModel(new String[]{"ID", "Pasien", "Dokter", "Obat", "Tanggal", "Keluhan", "Diagnosa", "Terapi", "Aksi"}, 0) {
            @Override
        ...
        table.getColumnModel().getColumn(8).setCellEditor(new ButtonEditor(table, new AksiEditHapus() {
            @Override
            public void onEdit(int row) {
                cbPasien.setSelectedItem(model.getValueAt(row, 1).toString());
                cbDokter.setSelectedItem(model.getValueAt(row, 2).toString());
                cbObat.setSelectedItem(model.getValueAt(row, 3).toString());
                tfTanggal.setText(model.getValueAt(row, 4).toString());
                taKeluhan.setText(model.getValueAt(row, 5).toString());
                taDiagnosa.setText(model.getValueAt(row, 6).toString());
                taTerapi.setText(model.getValueAt(row, 7).toString());
            }
    ...
    private void loadComboData() {
        try (Connection conn = KoneksiDB.getConnection()) {
            ...
            rs = stmt.executeQuery("SELECT * FROM obat");
            while (rs.next()) {
    ...
    private void simpanData() {
        try (Connection conn = KoneksiDB.getConnection()) {
            String sql = "INSERT INTO pemeriksaan (pasien_id, dokter_id, obat_id, tanggal, keluhan, diagnosa, terapi) VALUES (?, ?, ?, ?, ?, ?, ?)";
            PreparedStatement stmt = conn.prepareStatement(sql);
            stmt.setInt(1, pasienMap.get(cbPasien.getSelectedItem()));
            stmt.setInt(2, dokterMap.get(cbDokter.getSelectedItem()));
            stmt.setInt(3, obatMap.get(cbObat.getSelectedItem()));
            stmt.setString(4, tfTanggal.getText());
            stmt.setString(5, taKeluhan.getText());
            stmt.setString(6, taDiagnosa.getText());
            stmt.setString(7, taTerapi.getText());
            stmt.executeUpdate();
    ...
    private void tampilkanData() {
        model.setRowCount(0);
        try (Connection conn = KoneksiDB.getConnection()) {
            String sql = "SELECT p.id, pas.nama AS pasien, dok.nama AS dokter, oba.nama_obat AS obat, p.tanggal, p.keluhan, p.diagnosa, p.terapi " +
                         "FROM pemeriksaan p " +
